...
object ChutneyUtil {
    ...
    fun isChutneyJson(jsonPsi: PsiFile): Boolean {
        return jsonPsi.name.indexOf("scenario.json") > 1 || jsonPsi.name.indexOf("chutney.json") > 1 || isLegacyScenarioJson(jsonPsi)
    }

    fun isChutneyYaml(jsonPsi: PsiFile): Boolean {
        return jsonPsi.name.indexOf("chutney.yml") > 1 || jsonPsi.name.indexOf("chutney.yaml") > 1  || isLegacyScenarioYaml(jsonPsi)
    }
    ...
    fun getChutneyScenarioDescriptionFromFileName(fileName: String): String {
        return fileName.substring(0, fileName.indexOf(".chutney.json"))
    }
